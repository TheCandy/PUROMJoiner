...
// shape added to handle b-attribute object based on the shape in original puro modeler

go.Shape.defineFigureGenerator("bAttribute", function(shape, w, h) {
  var geo = new go.Geometry();
  var fig = new go.PathFigure(0, .5 * h, true);
  geo.add(fig);
  ...
  fig.add(new go.PathSegment(go.PathSegment.Line, w, .5 * h));
  fig.add(new go.PathSegment(go.PathSegment.Line, .75 * w, h));
  fig.add(new go.PathSegment(go.PathSegment.Line, .25 * w, h).close());
  return geo;
});

// Aliases to use shape as definition of type

go.Shape.defineFigureGenerator("bType", "Ellipse");
go.Shape.defineFigureGenerator("bRelation", "Diamond");
go.Shape.defineFigureGenerator("bValuation", "Parallelogram1");
go.Shape.defineFigureGenerator("bObject", "Rectangle");